...
if __name__ == "__main__":
    ...
    g, train_eids, valid_eids, test_eids, num_rels = modeling_v2.build_g_from_data(args.dataset_name, args.data_location, args.num_graph_nodes, args.num_rels, use_cuda = use_cuda)
    ...
    # load additional features (if any)
    img_features = None
    txt_features = None
    ...
    # send g and valid eid, test eid to GPU
    if use_cuda:
    ...
    if args.use_mlp == 'yes':
      # use gating mechanism to incorporate additional features
      use_mlp = True

    else:
      # don't use gating, concatenate as is OR no additional features used
      use_mlp = False

    model_MLP = models.model_train(train_g = train_g, valid_g = valid_g, test_g = test_g, g = g, valid_eids = valid_eids, test_eids = test_eids,
